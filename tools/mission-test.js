...
(async () => {
  ...
  await p.fill('#fName', 'David Massey');
  ...
  ok('name carried over', (await p.textContent('#ctxName')) === 'David Massey');
  ...
  ok('reload returns to the mission with no template', (await p.textContent('#ctxName')) === 'David Massey');
  ...
  ok('named from the mission', /^Trip_Report_Fort_Bliss_TX_Massey\.docx$/.test(dl.suggestedFilename()),
  ...
  ok('Upload Mission goes straight to the notes, no template needed',
     (await p2.textContent('#ctxName')) === 'David Massey');
  ...
  ok('generated file parses back as a template', who.length === 1 && /David Massey/.test(who[0]),
     JSON.stringify(who));
